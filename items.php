...
try {
  $stmt = $pdo->query("
    SELECT itemid, itemname, itemdesc, COALESCE(itemprice,0) AS itemprice, itemthumb,
      ...
      <?php foreach ($rows as $r): ?>
        <article class="rounded-2xl overflow-hidden bg-slate-900/50 border border-slate-800">
          <?php if (!empty($r['itemthumb'])): ?>
            <img src="<?= htmlspecialchars($r['itemthumb']) ?>"
          ...
          <div class="p-4 space-y-2">
            ...
            <div class="flex items-center justify-between pt-2">
              ...
              <div class="flex gap-2">
                <a href="/view_item.php?itemid=<?= (int)$r['itemid'] ?>"
                   class="rounded-md border border-slate-600 px-3 py-1.5 hover:bg-slate-800">View</a>
                <a href="/edit.php?itemid=<?= (int)$r['itemid'] ?>"
                   class="rounded-md border border-slate-600 px-3 py-1.5 hover:bg-slate-800">Edit</a>
              </div>
            </div>
            <?php if (!(int)$r['active']): ?>
